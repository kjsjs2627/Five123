...
public class OrderStatusServiceTest {
	...
	public void testA() {
		...
		try {
			output = orderStatusService.getOrderStatusItem(input);
	...
	public void testB() {
		...
		List<OrderStatus> output = null;
		
		try {
			output = orderStatusService.getOrderStatusList(input);
			
			for (OrderStatus item : output) {
	...
	public void testC() {
		...
		try {
			count = orderStatusService.getOrderStatusCount(null);
	...
	public void testE() {
		...
		try {
			output = orderStatusService.addOrderStatus(input);
			log.debug("저장된 데이터 수: " + output);
			// [중요] 생성된 PK값은 MyBatis에 의해 입력 파라미터의 해당 멤버변수에 셋팅된다.
			log.debug("생성된 PK값 : " + input.getOrder_status_no());
		} catch (Exception e) {
			log.error(e.getLocalizedMessage());
			e.printStackTrace();
		}
	...
	public void testF() {
		...
		try {
			output = orderStatusService.editOrderStatus(input);
	...
	public void testG() {
		...
		try {
			output = orderStatusService.deleteOrderStatus(input);
